...
* Khi mà các freed chunks quá lớn so với kích thước cho phép của tcache (> 0x410) và fastbin (> 0x80), hoặc khi mà ngăn xếp tcache và fastbin bị đầy, các chunk này sẽ được đẩy đến `unsortedbin`. Và các chunk ở trong `unsortedbin` có khả năng gộp vào nhau thành một chunk lớn hơn; và nếu các chunk này liền kề với top chunk thì sẽ được gộp với top chunk luôn.
```c
int main() {
...
bk_nextsize: 0x00
```
* Để có các chunk trong unsortedbin trỏ đến nhau, ta chen giữa các chunk 0x410 với các chunk khác size ở giữa. Ta thấy con trỏ fd (foward) và bk (backward) trỏ đến chunk tiếp theo và trỏ đến main_arena:
```c
int main() {
	char* p1 = malloc(0x410);
	char* p2 = malloc(0x20);
	char* p3 = malloc(0x410);
	char* p4 = malloc(0x20);
...
Addr: 0x555555559b30
prev_size: 0x00
size: 0x204d0 (with flag bits: 0x204d1)
fd: 0x00
bk: 0x00
fd_nextsize: 0x00
bk_nextsize: 0x00
...
pwndbg> bins
tcachebins
0x30 [  2]: 0x555555559b10 —▸ 0x5555555596c0 ◂— 0
fastbins
empty
unsortedbin
all: 0x5555555596e0 —▸ 0x555555559290 —▸ 0x7ffff7f8fb20 (main_arena+96) ◂— 0x5555555596e0
smallbins
empty
largebins
empty
...
0x5555555592a0: 0x00007ffff7f8fb20      0x00005555555596e0
pwndbg> x/4gx 0x5555555596e0
0x5555555596e0: 0x0000000000000000      0x0000000000000421
0x5555555596f0: 0x0000555555559290      0x00007ffff7f8fb20
pwndbg> x/4gx 0x7ffff7f8fb20
0x7ffff7f8fb20 <main_arena+96>: 0x0000555555559b30      0x0000000000000000
0x7ffff7f8fb30 <main_arena+112>:        0x00005555555596e0      0x0000555555559290
```
* Khi mà malloc() được gọi, yêu cầu cấp phát mà chunks trong tcache/fastbin không đáp ứng, thì sẽ tìm tiếp đến chunks trong unsortedbin. Nếu size mà malloc() yêu cầu cấp phát < size của chunk trong unsortedbin, thì chunk sẽ split ra sao cho đủ theo yêu cầu (bao gồm cả metadata). Và nếu malloc() yêu cầu size lớn hơn size của chunk trong unsortedbin, malloc() sẽ lấy chunk split từ topchunk, còn chunk  trong unsortedbin khi đấy sẽ được sắp xếp tướng ứng vào smallbin/largebin.
	* smallbin: 0x20 - 0x410
 	* largebin: > 0x410 
...
int main() {
	...
	char* p2 = malloc(0x410);
	malloc(0x20);
	// char* p5 = malloc(0x410);
	// char* p6 = malloc(0x20);
	// char* p7 = malloc(0x410);
...
Addr: 0x555555559b00
prev_size: 0x00
size: 0x20500 (with flag bits: 0x20501)
fd: 0x00
bk: 0x00
fd_nextsize: 0x00
bk_nextsize: 0x00
```
sau khi malloc(0x200):
```asm
...
Free chunk (unsortedbin) | PREV_INUSE
Addr: 0x5555555594a0
prev_size: 0x00
size: 0x630 (with flag bits: 0x631)
fd: 0x7ffff7f8fb20
bk: 0x7ffff7f8fb20
fd_nextsize: 0x00
bk_nextsize: 0x00
...
```
sau khi malloc(0x2000) lớn hơn so với chunk 0x630 trong unsortedbin
```asm
pwndbg> bins
tcachebins
empty
...
empty
largebins
0x600-0x630: 0x5555555594a0 —▸ 0x7ffff7f8ff90 (main_arena+1232) ◂— 0x5555555594a0
...
Free chunk (largebins) | PREV_INUSE   ///////// unsortedbin ban đầu --> đưa vào largebin
Addr: 0x5555555594a0
prev_size: 0x00
size: 0x630 (with flag bits: 0x631)
fd: 0x7ffff7f8ff90
bk: 0x7ffff7f8ff90
fd_nextsize: 0x5555555594a0
bk_nextsize: 0x5555555594a0
...
Allocated chunk | PREV_INUSE	/////////// cấp phát từ topchunk
Addr: 0x555555559b00
prev_size: 0x00
...
Top chunk | PREV_INUSE  //////////// topchunk bị giảm
Addr: 0x55555555bb10
prev_size: 0x00
size: 0x1e4f0 (with flag bits: 0x1e4f1)
fd: 0x00
bk: 0x00
fd_nextsize: 0x00
bk_nextsize: 0x00
...
https://www.youtube.com/watch?v=ZghaZXyihbM
